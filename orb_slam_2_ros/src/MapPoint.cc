...
MapPoint::MapPoint(const cv::Mat &Pos, KeyFrame *pRefKF, Map* pMap):
    ...
    mpReplaced(static_cast<MapPoint*>(NULL)), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap), mnSemanticLabel(0)
{
...
MapPoint::MapPoint(const cv::Mat &Pos, Map* pMap, Frame* pFrame, const int &idxF):
    ...
    mnFound(1), mbBad(false), mpReplaced(NULL), mpMap(pMap), mnSemanticLabel(0)
...
void MapPoint::ComputeDistinctiveDescriptors()
{
	...
	// to replace float Distances[N][N];
    ...
    for(size_t i=0;i<N;i++)
    {
        ...
        int median = vDists[round(0.5*(N-1))];
...
{
    std::lock_guard<std::mutex> lock_Pos(mMutexPos);
    std::lock_guard<std::mutex> lock_Features(mMutexFeatures);
