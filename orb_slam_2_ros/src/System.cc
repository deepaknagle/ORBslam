...
void System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const cv::Mat &imSeg, const double &timestamp)
{
    if(mSensor!=RGBD)
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageRGBD(im,depthmap,imSeg,timestamp);
